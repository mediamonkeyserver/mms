...
class SQLRegistry extends MemoryRegistry {
    ...
    getFiles(callback) {
        if (this._cached_files) {
            callback(null, this._cached_files);
        } else {
            this._cached_files = [];
            var paths = this._cached_files;
            this.sql.open("SELECT path FROM media", function (err, row) {
                if (!err)
                    paths.push(row.path);
            }, function () {
                callback(null, paths);
    ...
    deleteMetas(path, callback) {
        ...
        var sync_id = this._getSyncId(path);
        var sql = 'DELETE FROM media WHERE sync_id = "'+sync_id + '"';
        this.sql.exec(sql, callback);
    }
    ...
    putConfig(config, callback) {
        this._putCollections(config.collections, ()=>{
            ...
            fs.writeFile( path, JSON.stringify(_configCopy), callback);
        });
    }
}
