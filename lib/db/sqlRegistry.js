...
class SQLRegistry extends MemoryRegistry {
    ...
    getPlaylists(parent_guid, callback) {
        ...
        sql.open("SELECT * FROM playlists WHERE parent_guid" + q_parent_guid, function (err, row) {
            ...
            if (parent_guid) {
                var tracks = [];
                sql.open("SELECT * FROM playlist_items_map WHERE playlist_guid" + q_parent_guid + ' ORDER BY play_order', function (err, row) {
                    if (!err)
        ...
        this.sql.open("SELECT guid FROM playlists WHERE guid = '" + item.guid + "'", function (err, row) {
            ...
            q.finalize(function () {
                if (item.tracks) {
                    ...
                    var qf = this.sql.prepare('INSERT INTO playlist_items_map (playlist_guid, item_sync_id, play_order) VALUES (?,?,?)');
                    for (var i = 0; i < tracks.length; i++)
                        qf.run(item.guid, tracks[i], i);
                    qf.finalize(callback);
                } else {
                    callback();
                }
            }.bind(this));
