...
class SQLRegistry extends MemoryRegistry {
	...
	putPlaylist(item, service, callback) {
		...
		this.sql.open('SELECT guid FROM playlists WHERE guid = \'' + item.guid + '\'', ( /*err, row*/ ) => {
			...
			var sql;
			if (item_exists)
				sql = 'UPDATE playlists SET title = ?, parent_guid = ?, last_modified = ? WHERE guid = ?';
			else
				sql = 'INSERT INTO playlists (title, parent_guid, last_modified, guid) VALUES (?,?,?,?)';

			var q = this.sql.prepare(sql);
			q.run(item.name, item.parent_guid, item.last_modified, item.guid /*needs to be the last*/ );
			q.finalize(() => {
