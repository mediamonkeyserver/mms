...
class SQLRegistry extends MemoryRegistry {
    ...
    _getSyncId(path) {
        return this._removeFileExt(path.replace(/\\/g, '/').toLowerCase());
    }
    ...
    getPlaylists(parent_guid, callback) {
        ...
        _this.sql.open("SELECT * FROM playlists WHERE parent_guid" + q_parent_guid, function (err, row) {
            ...
            if (parent_guid) {
                var tracks = [];
                _this.sql.open("SELECT * FROM playlist_items_map WHERE playlist_guid" + q_parent_guid + ' ORDER BY play_order', function (err, row) {
                    if (!err)
        ...
        this.sql.open("SELECT guid FROM playlists WHERE guid = '" + item.guid + "'", function (err, row) {
            ...
            q.finalize(() => {
                ...
                for (var i = 0; i < tracks.length; i++) {
                    ...
                    path = service.removeLastSlash(repository.originalPath) + path;
    ...
    getConfig(config, callback) {
        var _default_cleared;
        this.sql.open("SELECT * FROM collections", function (err, row) {
            if (err) {
                callback(err);
            } else {
                if (!_default_cleared) { // any collection exists, clear the default collections
                    config.collections = [];
    ...
    _putCollection(item, callback) {
        var item_exists = false;
        this.sql.open("SELECT * FROM collections WHERE id = '" + item.id + "'", (err, row) => {
            item_exists = true;
        }, () => {
            ...
            var sql;
            if (item_exists)
                sql = 'UPDATE collections SET title = ?, config = ? WHERE id = ' + item.id;
            else
    ...
    putConfig(config, callback) {
        ...
        var list = config.collections || [];
        var _processItem = function (idx) {
            if (idx >= list.length) {
                if (callback)
                    callback();
            } else {
                this._putCollection(list[idx], () => {
                    _processItem(idx + 1);
                });
            }
        }.bind(this);
