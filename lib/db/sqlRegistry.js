...
class SQLRegistry extends MemoryRegistry {
	...
	prepareStatements(callback) {
		...
		const sqlStatements = {
			...
			deleteCollectionsByIdList: 'DELETE FROM collections WHERE _id NOT IN (?)',
			...
			getPlaylistsByGuid: 'SELECT * FROM playlists WHERE guid IN (?)',
	...
	getPlaylistsBy(params) {
		...
		if (params.guidArray.length != 0) {
			var rows = this.sql.getStatement('getPlaylistsByGuid').all();
	...
	updateUser(userId, userData) {
		...
		return this.sql.getStatement('updateUser').run(name, display_name, role_key, password);
	}
	...
	getRoles() {
		...
		return this.sql.getStatement('getRoles').get();
	}
	...
	_putCollections(collections, callback) {
		...
		var _processItem = function (idx) {
			if (idx >= list.length) {
				//console.log(ids);
				if (ids.length) {
					this.newSql.getStatement('deleteCollectionsByIdList').run(ids.join(','));
				}
				else
					callback();
			} else {
