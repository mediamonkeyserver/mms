...
class SQLRegistry extends MemoryRegistry {
	...
	prepareStatements(callback) {
		...
		const sqlStatements = {
			...
			getPlaylistByGuid: 'SELECT * FROM playlists WHERE guid = ?',
			getPlaylistsByParentGuid: 'SELECT * FROM playlists WHERE parent_guid = ?',
			getPlaylistsByParentGuidRoot: 'SELECT * FROM playlists WHERE parent_guid IS NULL',
	...
	getPlaylistContent(parent_guid, callback) {
		...
		if (parent_guid)
			playlistRows = this.sql.getStatement('getPlaylistsByParentGuid').all(parent_guid);
		else
			playlistRows = this.sql.getStatement('getPlaylistsByParentGuidRoot').all();
	...
	getPlaylistsBy(params) {
		...
		if (params.guidArray.length != 0) {
			for (var guid of params.guidArray) {
				var rows  = this.sql.getStatement('getPlaylistByGuid').all(guid);								
				if (rows && rows[0]) 
					playlists.push(this._playlistFromRow(rows[0]));				
			}
		}
	...
	getLastContentToken() {
		...
		var token = this._getLastContentToken(true);
