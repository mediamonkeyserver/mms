		...
		for (var key in attr) {
			...
			if (key == 'albumArts') {
				if (attr.albumArts.length) {
					...
					track.artworkURL = contentPath + node_id + '/' + first.contentHandlerKey + '/0';
				}
			} else {
				if (addField) {
					var at = attr[key];
		...
		this.getFiles((err, files) => {
			...
			for (var f of files) {				
				if (this._fileIsInFolders(f, params.folders)) {
					...
					var track = this._getTrackEntryFromFile(f, filter_fields);
					res.push(track);					
				}				
			}
