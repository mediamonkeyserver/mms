...
class MediaProvider extends Cacher {
	...
	processImagesUpdate(request, response) {

		var params = request.body;

		if (!(params.db_id || params.sync_id))
			return response.status(400).end('Either db_id or sync_id needs to be specified!');
		else {
			this.registry.getFile({
				id: params.db_id,
				...
				if (err)
					...
					for (var img of params.images) {
						var mt = img.mimeType || this._getMime(img.path);
