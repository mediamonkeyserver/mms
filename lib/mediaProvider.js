...
class MediaProvider extends Cacher {
	...
	_getResources(file) {
		...
		if (!file.albumArts || !file.albumArts.length) {
			// temporal hack for files that haven't been scanned by music-metadata, but imported from MM5
			// TODO: import the tag album art info from MM5
			file.albumArts = [{
	...
	getDefaultFolderForMime(mt /*mime type*/ ) {
		...
		if (col && col.folders.length)
			return col.folders[0];
		else {
			// when there is no collection (or no folder in collection)
			if (mt.startsWith('audio'))
				return PathNormalizer.normalize(os.homedir()) + '/Music';
			else
				return PathNormalizer.normalize(os.homedir()) + '/Videos';
		}
	}
