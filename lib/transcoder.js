...
class Transcoder {
	...
	static async getStreamInfo(mediaItem, request) {
		return new Promise((resolve/*, reject*/) => {
			const accepted = ['audio/mpeg', 'video/mpeg', 'video/mp4', 'audio/mp4', 'audio/mp3', 'video/webm', 'audio/webm', 'video/ogg', 'audio/wav', 'audio/ogg'];
			request = request || {};
			...
			if (userAgent.startsWith('MediaMonkey')) {
				...
				if (!forceHLS) {
					...
					if(userAgent.toUpperCase().includes('CRKEY')) {
						// it is Chromecast
						var acceptedChr = ['audio/mp4', 'video/mp4', 'audio/mp3', 'audio/mpeg', 'audio/webm', 'video/webm', 'audio/wav', 'audio/aac'];
				...
				if (forceHLS || !resMime) {
					if(isChromecast) {
						resMime = 'application/dash+xml'; // not supported yet
					}
					else
