...
class Transcoder {
	...
	static async getStreamInfo(mediaItem, reqQuery) {
		return new Promise((resolve/*, reject*/) => {
			const accepted = ['audio/mpeg', 'video/mpeg', 'video/mp4', 'audio/mp4'];

			var transcode = false;
			var forceHLS = (reqQuery && reqQuery.forceHLS);
	...
	static async convert(contentURL, mediaItem, reqQuery) {

		return new Promise(async (resolve, reject) => {

			var streamInfo = await Transcoder.getStreamInfo(mediaItem, reqQuery);
