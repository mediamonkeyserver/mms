...
const mkdirp = require('mkdirp'); // Can create multiple nested folders
...
class Transcoder {
	...
	async createM3UStream(duration, tokenAdd) {
		var s = '';
		s += '#EXTM3U\n';
		s += '#EXT-X-VERSION:3\n';
		...
		for (var i = 0, done = 0; done < duration; i++, done += TARGETDURATION) {
			...
			s += `/api/trans/${this.uuid}/${OUT_FILE_PREFIX}${i}.${OUT_FILE_EXT}${tokenAdd}\n`;
		}
		...
		return new Promise(async (resolve, reject) => {
			...
			if (trans.mimeType === 'application/x-mpegurl') {
				...
				if (request.query && request.query.token)
					tokenAdd = '?token=' + request.query.token;
				trans.targetArgs = [
				...
				trans.outStream = await trans.createM3UStream(mediaItem.duration, tokenAdd);
				resolve(trans);
			} else {
