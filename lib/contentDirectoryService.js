...
class ContentDirectoryService extends Service {
	constructor(configuration) {
		...
		this._searchableFields = ['dc:title', 'upnp:genre', 'upnp:artist', 'upnp:albumArtist', 'upnp:author', 'upnp:album', 'upnp:actor', 'upnp:director', 'upnp:producer', 'upnp:publisher'];
	...
	doFullTextSearch(search, callback) {
		...
		for (var f of search.fields) {
			var db_field = f.field.substring(f.field.indexOf(':') + 1); // strip 'dc:' and 'upnp":' prefixes
			...
			if (phrase != '')
				phrase = phrase + ' OR ';
			phrase = phrase + db_field + ': ' + f.value;
