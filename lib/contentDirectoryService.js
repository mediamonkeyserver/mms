...
class ContentDirectoryService extends Service {
		...
		this._nodeRegistry.registerRepository(repository, hashKey, (error, repository) => {
			...
			this._installRoot((error, root) => {
				...
				repository.initialize(this, (error) => {
                    ...
                    this.rescanPath(Path.join(os.homedir(), '/Music/')); // LS: to re-scan default location for uploading files (TODO: to be stored in database)
	            ...
	            request.on('end', () => {
	                if (body == '') {
	                    ...
	                    this.nodeRegistry.getMetas(path, null, (error, metas) => {
	                        if (error)
	                            return _failure('Old metadata fetch has failed:' + error);
	                        else {
	                            ...
	                            this.nodeRegistry.putMetas(path, null, metas, (error) => {
	                                if (error) {
	                                    return _failure('Update failed, reason: ' + error);
	                                } else {
	...
	createUploadSession(action, request, response, callback) {
	    ...
	    request.on('end', () => {
	        ...
	        var path = Path.join(os.homedir(), content.path);
	        ...
	        if (content.metas) {
	            var new_metas = content.metas;
	            _this.nodeRegistry.getMetas(path, null, (error, metas) => {
	                ...
	                if (metas) { // this track already exists, merge the old metas with new metas:                                           
	                    for (var key in new_metas) {
	                        metas[key] = new_metas[key];
	                    }
	                } else {
	                ...
	                // update the new metas into database:
	                _this.nodeRegistry.putMetas(path, null, metas, (error) => {
	                    if (error) {
	                        return _failure('Upload failed, metadata error: ' + error);
	                    } else {
	                        _createSession();
	                    }
	                });
	...
	rescanPath(path) {
	    logger.info('Scanning of ' + path + ' has started');
	    ...
	    repository.scan(this, repository._mountPathNode, (error) => {
	        if (error)
	            ...
	            logger.info('Scanning of ' + path + ' finished');
	    });
	}
