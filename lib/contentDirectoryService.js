...
class ContentDirectoryService extends Service {
	...
	_extractBetween(from, to, value) {
		var re = new RegExp('(?:' + from + ')(.*?)(?:' + to + ')', 'ig'); //set ig flag for global search and case insensitive
	...
	_extractSearchField(field, operator, value) {
		var terms = this._extractBetween(field + ' ' + operator + ' "', '"', value);
		if (terms)
			return this._newSearchField(field, operator, terms[terms.length - 1]);
	}
	...
	parseSearchCriteria(value) {
		...
		res.fields = [];
		this._parseSearchField('dc:title', value, res.fields); // TODO - all fields		
