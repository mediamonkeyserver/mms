...
class ContentDirectoryService extends Service {
		...
		Configuration.setRegistry(this.nodeRegistry, () => {
			...
			observer.on('collectionchange', () => {
				this._rescanCollections(config.collections, {}, () => {
					this._garbageNonCollectionFiles(config.collections);
				});
			});
	...
	_rescanCollections(collections, scan_options, callback) {
		...
		var _processItem = function (idx) {
			if (idx >= list.length) {
				...
				this._rescanCollection(list[idx], scan_options, () => {
					_processItem(idx + 1);
				});
			}
		}.bind(this);
	...
	_garbageNonCollectionFiles(collections, callback) {
		...
		this.nodeRegistry.garbageFilesOutOfFolders(folders, callback);
	}
	        ...
	        if (!node.contentURL) {
	            ...
	            request.on('end', () => {
	                if (body == '') {
						...
						logger.info('Request to update metadata for: ' + node.name + ', path ' + path + ', ' + node.upnpClass);
	                    logger.verbose('Update data: ' + body);
