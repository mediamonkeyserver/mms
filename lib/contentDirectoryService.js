...
class ContentDirectoryService extends Service {
	...
	_initUIConfigObserver(callback) {
		...
		observer.on('collectionchange', () => {
			this._rescanCollections(config.collections);
		});
		this._rescanCollections(config.collections, () => {
			this._getRepositoryForCollection({ // to add predefined 'Playlists' collection for playlists browsing
				name: 'Playlists',
				type: 'playlist'
			}, callback);
		});		
	...
	_rescanCollections(collections, callback) {
		...
		var _processItem = function (idx) {
			if (idx >= list.length) {
				...
				this._rescanCollection(list[idx], () => {
					_processItem(idx + 1);
				});
			}
		}.bind(this);
	...
	_getRepositoryForCollection(collection, callback) {
		...
		if (repository) {
			...
			if (collection.folders && collection.folders.length)
				defaultPath = this.normPath(collection.folders[0]);
	...
	_rescanCollection(collection, callback) {
		this._getRepositoryForCollection(collection, (repository) => {
			...
			var _processItem = function (idx) {
				if (idx >= list.length) {
					...
					var path = list[idx];
					this.rescanPath(path, repository, () => {
						_processItem(idx + 1);
					});
				}
			}.bind(this);
	...
	getNodeByUid(uid, callback) {
		this._nodeRegistry.getNodeByUid(uid, callback);
	}	
	...
	processRequest(request, response, path, callback) {
		...
		switch (segment) {
            ...
            case "update":
                // LS: custom updating of node metadata (e.g. track metadata update)											
                this.processNodeUpdate(action, request, response, callback);
                return;
                
            case "createUploadSession":                
                this.createUploadSession(action, request, response, callback);
                return;  
                
            case "upload":                
                this.processUpload(action, request, response, callback);
				return;
				
			case "uploadPlaylist":
                this.processPlaylistUpload(action, request, response, callback);
                return;
		}
	...
	processNodeUpdate(action, request, response, callback) {
	    ...
	    this.getNodeById(id, (error, node) => {
	        ...
	        if (!node || !node.id)
	            return _failure('Node #' + id + ' not found');

	        if (!node.contentURL) {
	            _failure('Update unsupported for this node: ' + node.upnpClass + ', name:' + node.name);
	        } else {
	            var body = '';
	            request.on('data', (data) => body += data);
	            request.on('end', () => {
	                if (body == '') {
	                    return _failure('No request body, use POST request with JSON content');
	                } else {

	                    try {
	                        var new_metas = JSON.parse(body);
	                    } catch (e) {
	                        return _failure('Failure to parse JSON: ' + e + ' : ' + body);
	                    }

	                    logger.info('Request to update node: ' + node.upnpClass + ', name:' + node.name);
	                    logger.info('Update data: ' + body);

	                    var path = node.contentURL.path;
	                    this.updateMetas(path, new_metas, node, (error) => {
	                        if (error) {
	                            return _failure(error);
	                        } else {
	                            response.writeHead(200, 'OK');
	                            response.end('Updated node: ' + node.upnpClass + ', name:' + node.name);
	                            callback(null, true);
	                        }
	                    });
	                }
	...
	updateMetas(path, new_metas, node /* optional */ , callback) {
	    this.nodeRegistry.getMetas(path, null, (error, metas) => {
	        if (error)
	            ...
	            this.nodeRegistry.putMetas(path, null, metas, (error) => {
	                if (error) {
	...
	createUploadSession(action, request, response, callback) {
	    ...
	    request.on('end', () => {
	        ...
	        var path = this.normPath(content.path);
	        logger.info('createUploadSession: ' + path);


	        var sessionID = Math.random().toString(36).replace(/[^a-z]+/g, '').substr(0, 6);

	        _this._uploadSessions = _this._uploadSessions || {};
	        _this._uploadSessions[sessionID] = {
	            path: path,
	            metas: content.metas
	        }
	...
	processUpload(action, request, response, callback) {

	    var parameters = action.split('/');
	    var upload_id = parameters.shift();
		...
		var repository = this.getRepositoryForPath(path);
		path = this.removeLastSlash(repository.originalPath) + path;
	        ...
	        if (error) {
	            ...
	            request.on('end', () => {
	                ...
	                if (endBytes == totalBytes || endBytes == totalBytes - 1) { // LS: this is the last upload part, re-scan the directory:
	                    ...
	                    _this.rescanPath(dir, repository, (error) => {
	                        if (!error && session.metas) {
	                            _this.updateMetas(path, session.metas, null, (error) => {
	                                if (error)
	                                    logger.error(error);
	                                else
	...
	processPlaylistUpload(action, request, response, callback) {
	    ...
	    request.on('end', () => {
			...
			logger.info('playlistUpload: ' + body);
			...
			this.nodeRegistry.putPlaylist(content, this, (err) => {
				if (err) {
					_failure(err);
				} else {
					response.writeHead(200, 'OK');
    ...
    rescanPath(path, repository, callback) {
        path = this.normPath(this.removeLastSlash(path));
		var dt = Date.now();
