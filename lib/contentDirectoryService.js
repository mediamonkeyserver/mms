...
class ContentDirectoryService extends Service {
	...
	removeNode(node, callback) {
		this.getNodeById(node.parentId, (err, parent) => {
			if (!err && parent)
				parent.removeChild( node, callback); // this also removes node.id from parent.childrenIds array
			else
				this.unregisterNode( node, callback);
		});
	}
	...
	processNodeDeletion(node, request, response, parameters, callback) {
		...
		if (!node.isUpnpContainer) {
			...
			fs.unlink(path, (err) => {
				if (!err) {
					this.nodeRegistry.deleteMetas(path);
			...
			if (node.upnpClass.name == 'object.container.playlistContainer') {
				this.nodeRegistry.deletePlaylist({ guid: node.guid }, (err) => {
					if (!err) {
						this.removeNode(node, () => {
							response.writeHead(200, 'OK');
							response.end();
				...
				response.writeHead(400, 'Bad Request');
				var err = 'Deletion not supported for class ' + node.upnpClass.name;
				response.end(err);
				callback(err, true);
			}
		}
