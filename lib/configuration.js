...
class Configuration {
	...
	getProvider() {
		assert(_mediaProvider, 'Media provider not initialized! , setRegistry() not called?');
	...
	setRegistry(registry, callback) {
		...
		_mediaProvider = new MediaProvider(_registry);
	}

	_saveToRegistry(config) {		
		this.getRegistry().putConfig(config, () => {});
	}
