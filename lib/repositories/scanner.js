...
class ScannerRepository extends PathRepository {
	...
	scan(service, node, callback) {
		...
		if (this.onDirectoryScanStart)
			this.onDirectoryScanStart(infos);
		...
		this._scanDirectory(node, infos, files, (error) => {
			...
			logger.info('Scan ' + this.type + ' directory: ' + infos.contentURL + ', ' + files.length + ' files to process');
			...
			Async.eachLimit(files, FILES_PROCESSOR_LIMIT, (infos, callback) => {
				...
				this.processFile(node, infos, (error) => {
					if (error) {
						logger.warn('Process file error: ', error, 'Path: ' + infos.contentURL);
					}
				...
				if (this.onDirectoryScanEnd)
					this.onDirectoryScanEnd(error, infos, files);
				...
				//logger.info('Scan:', files.length, 'files processed');
