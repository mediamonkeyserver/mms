...
class MusicRepository extends ScannerRepository {
	...
	onDirectoryScanStart( /*infos*/ ) {
		this._scannedDirs = {};
	}

	onDirectoryScanEnd( /*err, infos*/ ) {
	...
	processFile(rootNode, infos, callback) {
		...
		var mt = infos.mimeType || infos.contentURL.mimeLookup();
		if (mt.startsWith('image')) {
			this._addImage2LinkTracks(infos);
			return callback();
		}

		if (!mt.startsWith('audio') || !this.keepFile(infos)) {
			callback('Unsupported mime:  ' + mt);
			return;
		}
