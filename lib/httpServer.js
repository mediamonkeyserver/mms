	...
	getLocalIP() {
		...
		Object.keys(ifaces).forEach(function (ifname) {
			ifaces[ifname].forEach(function (iface) {
				...
				// Guess the most common local address
				var val = 0;
				...
				if (name.startsWith('e') || name.startsWith('w')) // A hack that tries to guess 'ethernet' or 'wifi' networks
					val += 10;

				if (val > bestVal) {
					bestVal = val;
					bestIf = iface.address;
				}
			});
		});

		return bestIf || '127.0.0.1';
	}
