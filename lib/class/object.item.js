...
class Item extends ObjectClass {
	...
	toJXML(node, attributes, request, filterCallback, callback) {
		...
		if (filterCallback(Xmlns.PURL_ELEMENT, 'date')) {
			...
			if (attributes.year) {
				var y = attributes.year;
				var _date;
				if (y < 10000) {					
					_date = y + '-01-01'; // appendix because of Mantis issues #13198, #11882
				} else {
					var ys = y.toString();
					_date = ys.substr(0, 4) + '-' + ys.substr(4, 2) + '-' + ys.substr(6, 2);
				}
				content.push({
					_name: 'dc:date',
					_content: _date
		...
		if (filterCallback(Xmlns.UPNP_METADATA, 'artist')) {
			...
			if (attributes.albumArtists)
				Item.addList(content, Item.addRole(attributes.albumArtists, 'AlbumArtist'), 'upnp:artist', true);
		}
		...
		if (filterCallback(Xmlns.UPNP_METADATA, 'publisher')) {
			Item.addList(content, attributes.publishers, 'upnp:publisher', false);
		}
		...
		if (filterCallback(Xmlns.UPNP_METADATA, 'longDescription')) {
			if (attributes.comment)
				content.push({
					...
					_content: attributes.comment
				});
		}

		// unofficial attributes:  
		if (filterCallback(Xmlns.UPNP_METADATA, 'PlayCounter')) {
			if (typeof (attributes.playcount) === 'number') {
				content.push({
					_name: 'upnp:PlayCounter',
					_content: attributes.playcount
				});
			}
		}

		if (filterCallback(Xmlns.UPNP_METADATA, 'MM_Bookmark')) {
			if (typeof (attributes.bookmark) === 'number') {
				content.push({
					_name: 'upnp:MM_Bookmark',
					_content: attributes.bookmark
				});
			}
		}

		if (filterCallback(Xmlns.VENTIS_METADATA, 'metajson')) {
			Item.addNamespaceURI(xml, 'ventis', Xmlns.VENTIS_METADATA);
			var toJSON = {};
			var fields = ['rating', 'playcount', 'skipcount', 'bpm', 'bookmark', 'volumeLeveling', 'volumeLevelTrack', 'volumeLevelAlbum', 'last_time_played', 'parental_rating', 'grouping', 'tempo', 'mood', 'occasion', 'quality', 'isrc', 'initialKey', 'conductor', 'contributor', 'lyricist', 'originalTitle', 'originalArtist', 'originalLyricist', 'originalDate', 'copyright', 'encoder', 'subtitle', 'custom1', 'custom2', 'custom3', 'custom4', 'custom5', 'custom6', 'custom7', 'custom8', 'custom9', 'custom10'];
			fields.forEach((n) => {
				if (attributes[n]) {
					toJSON[n] = attributes[n];
				} else
				if (attributes[n + 's']) {
					toJSON[n] = attributes[n + 's'].join(';'); // like lyricists, conductors
				}
			});
			var _json = JSON.stringify(toJSON);
			content.push({
				_name: 'ventis:metajson',
				_content: Item.encodeHTML(_json)
			});
		}
