...
class Audio_MusicMetadata extends ContentHandler {
			...
			mm.parseFile(contentURL.path, {
				...
				if (tags.picture) {
					...
					tags.picture.forEach((picture) => {
						var mimeType = PathNormalizer.getMime(picture.format) || picture.format;
	...
	_parseID3(ID3, metas) {
		for (var item of ID3) {
			if (item.id == 'TSRC')
				metas.isrc = item.value;
			else
			if (item.id == 'TKEY')
				metas.initialKey = item.value;
			else
			if (item.id == 'USLT')
	...
	processRequest(node, request, response, path, parameters, callback) {
		...
		this._getPicture(node, contentURL, albumArtKey, (error, picture) => {
			...
			if (error || !picture.format || !picture.data) {
				/*
				let error = new Error('Invalid picture for node #' + node.id + ' key=' + albumArtKey);
				error.node = node;
				error.request = request;
				*/				
				return callback(null, false /* not processed => 404 in api.js*/);
			}
	...
	_getPicture(node, contentURL, pictureIndex, callback) {
		...
		mm.parseFile(contentURL.path, {
			...
			if (metadata.common.picture && metadata.common.picture.length > pictureIndex)
				...
				var err = 'Cannot find picture index ' + pictureIndex;
				logger.debug(err);
				callback(err);
			}
		}).catch((err) => {
