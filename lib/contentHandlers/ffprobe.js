...
const metadataMapping = { // mapping read tag in lowercase to metas property
	title: 'title',
	album: 'album',
	show: 'album',	
	artist: 'artists',
	...
	'wm/conductor': 'conductors',
	'involved people':'contributors',
	lyricist: 'lyricists',
	'wm/writer': 'lyricists',
	'original title': 'originalTitle',
	'original artist': 'originalArtist',
	'original lyricist': 'originalLyricist',
	'wm/originalalbumtitle': 'originalTitle',
	...
	usercustom8: 'custom8',
	usercustom9: 'custom9',
	usercustom10: 'custom10'	
};

var multiValuedMetas = {
	artists: true,
	albumArtists: true,
	actors: true,
	authors: true,
...
class ffprobe extends Abstract_Metas {
	...
	_processProbe(json, callback) {
		...
		if(json.format.tags) {
			var tgs = json.format.tags;
			Object.keys(tgs).forEach(function (key) {
				var keyLC = key.toLowerCase();
				var m = metadataMapping[keyLC];
				if(m) {
					if(multiValuedMetas[m])
						metas[m] = tgs[key].split(';').map((a) => (a.trim()));
					else
						metas[m] = tgs[key];
				}
			});
			
			// date and original date to our year format
			if(tgs.date) {
				metas.year = convertDate(tgs.date);
			} else (tgs['WM/Year'])
			if(tgs['ORIGINAL DATE']) {
				metas.originalDate = convertDate(tgs['ORIGINAL DATE']);
			} else if(tgs['original date']) {
				metas.originalDate = convertDate(tgs['original date']);
			}
		}
		callback(null, metas);
...
function convertDate(dtStr) {
	...
	return 10000*dt.getFullYear()+100*(dt.getMonth()+1)+dt.getDate();
}
