...
class FileContentProvider extends ContentProvider {
   ...
   */
	stat(path, callback) {
		...
		fs.stat(osPath, (error, stats) => {
			...
			var mimeType = PathNormalizer.getMime(path) || '';
			...
			if (error) {
				...
				if (error.code === 'ENOENT') {
					...
					this._mkdir(parent, (error) => {
						if (error) {
							logger.info('_mkdir', 'parent=', osPath, 'error=', error);
							return callback(error);
						}
					...
					logger.info('_mkdir', 'parent=', osPath, 'access problem=', error);
				}
   ...
   */
	createWriteStream(url, options, callback) {
		...
		this._mkdir(parent, (error) => {
			if (error) {
				logger.info('createWriteStream', 'parent=', parent, 'access problem=', error);
				return callback(error);
			}
