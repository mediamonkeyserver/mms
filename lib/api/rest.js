	...
	constructor() {
		...
		this.get('/playlists/:guid', (req, res) => {
			...
			if (parent_guid == '0') // root
				parent_guid = null;
			Configuration.getRegistry().getPlaylistContent(parent_guid, (err, playlists, tracks) => res.json({ playlists: playlists, tracks: tracks }));
		});		
		...
		this.get('/get_last_content_token/', (req, res) => {
			Configuration.getRegistry().getLastContentToken((token) => {
				res.json({token: token});
			});
		});

		this.get('/get_content_changes/:token', (req, res) => {
			Configuration.getProvider().getContentChanges(req.params.token, (err, changes) => {
				if (err) {
