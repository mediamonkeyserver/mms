...
router.get('/folders', Auth.authorize(process.env.ACCESS_ADMIN), (req, res) => {
	var lpath = String(req.query.path); //2020-09-12 JL: forced req.query.path to be string to avoid TS error
...
router.get('/tracks/:collectionId', Auth.authorize(process.env.ACCESS_VIEWER), (req, res) => {
	...
	if (req.query.filter)
		params.filters = JSON.parse(String(req.query.filter));
	if (req.query.search)
...
router.get('/stream/:id', Auth.authorize(process.env.ACCESS_VIEWER), (req, res) => {
	...
	if (ext === 'm3u8') {
		req.query = req.query || {};
		req.query.forceHLS = String(true);
	}
...
router.get('/last-content-token', Auth.authorize(process.env.ACCESS_ADMIN), (req, res) => {
	var token = MediaProvider.registry.getLastContentToken();
	res.json({
		token: token
	});
});
