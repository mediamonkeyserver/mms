	...
	constructor() {
		...
		this.get('/playlists/:guid', (req, res) => {
			...
			if (parent_guid == '0') // root
				parent_guid = null;
			MediaProvider.registry.getPlaylistContent(parent_guid, (err, playlists, tracks) => res.json({ playlists: playlists, tracks: tracks }));
		});		
		...
		this.post('/players/:id/seek/:newTime', (req, res) => {
		...
		this.post('/upload-session', (req, res) => {			
		...
		this.get('/trans/:id/:file', (req, res) => {
			...
			if (trans)
				...
				res.json({error: 'Transcoding session doesn\'t exist'});
			}
		});

		this.get('/last-content-token', (req, res) => {
			MediaProvider.registry.getLastContentToken((token) => {
				res.json({token: token});
			});
		});

		this.get('/content-changes/:token', (req, res) => {
